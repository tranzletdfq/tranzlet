...
const loadTags = async () => {
  ...
  container.querySelectorAll('[data-approve]').forEach(button=>button.addEventListener('click',async()=>{button.disabled=true;button.textContent='Processing…';const {error:rpcError}=await supabase.rpc('approve_tag_and_credit_wallet_atomic',{p_tag_id:button.dataset.approve});if(rpcError){message(rpcError.message,true);button.disabled=false;button.textContent='Approve & credit';return;}message('Tag approved and the customer wallet was credited.');await loadTags();}));
...
const loadHandles = async () => {
  ...
  container.querySelectorAll('form').forEach(form=>form.addEventListener('submit',async event=>{event.preventDefault();const button=form.querySelector('button');const body=new FormData(form);button.disabled=true;try{const payload={asset_type:form.dataset.route,handle_name:String(body.get('handle_name')).trim(),handle_value:String(body.get('handle_value')).trim(),is_active:body.get('active')==='on',updated_at:new Date().toISOString()};let result;if(form.dataset.id) result=await supabase.from('company_payment_handles').update(payload).eq('id',form.dataset.id);else result=await supabase.from('company_payment_handles').insert([payload]);if(result.error)throw result.error;message(`${form.dataset.route} payment details saved.`);await loadHandles();}catch(error){message(error.message||'Unable to save payment details.',true);}finally{button.disabled=false;}}));
};
